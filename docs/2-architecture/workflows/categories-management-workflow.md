...
sequenceDiagram
    ...
    alt صورة مرفقة
        ...
        C->>MS: معالجة الصورة محلياً وضغط WebP
        C->>MS: رفع للتخزين السحابي
...
stateDiagram-v2
    ...
    رفع_الصورة --> معالجة_الصورة: تحسين وضغط محلياً
    ...
    صورة_محسنة --> فئة_نشطة: جاهزة للاستخدام
...
graph TB
    ...
    ImageUpload -->|نعم| ProcessImage[معالجة الصورة محلياً]
    ...
    SaveCategory --> CategoryActive[فئة نشطة]
    ...
    DataError --> End([نهاية])
    ImageError --> CategoryActiveNoImage[فئة نشطة بدون صورة]
    CategoryActiveNoImage --> CategoryActive
    SaveUpdate --> CategoryActive
    UpdateNewOrder --> CategoryActive
    DeletionComplete --> End
```
...
```typescript
async function uploadCategoryImageToMinio(
  categoryId: string,
  merchantId: string,
  ...
  if (file.size > maxBytes) {
    await unlink(file.path).catch(() => null);
    throw new BadRequestException('حجم الصورة كبير جداً');
  }

  // 2. معالجة الصورة محلياً
  const processedPath = await this.encodeSquareWebpUnder2MB(file.path, maxBytes);

  // 3. رفع للتخزين السحابي MinIO
  ...
  try {
    ...
    // 4. تحديث الفئة
    const category = await this.repo.findByIdForMerchant(categoryId, new Types.ObjectId(merchantId));
    if (category) {
      category.image = url;
      await category.save();
    }
...
// منع حذف الفئة التي تحتوي على فئات فرعية بدون cascade
const subtreeIds = await categoriesRepo.findSubtreeIds(merchantId, categoryId);
if (subtreeIds.length > 1 && !cascade) {
  throw new BadRequestException('Category has children. استخدم ?cascade=true لحذف الشجرة.');
}
```
