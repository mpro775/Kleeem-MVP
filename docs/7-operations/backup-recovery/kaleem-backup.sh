#!/usr/bin/env bash
# kaleem-backup.sh — unified backup + retention for Kaleem VPS
# Usage: sudo bash kaleem-backup.sh
set -euo pipefail
...
# Optional Healthchecks URL (ping on start/success/fail)
HEALTHCHECKS_URL="${HEALTHCHECKS_URL:-}"   # e.g. https://hc-ping.com/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx

# MongoDB
MONGO_URI="${MONGO_URI:-mongodb://127.0.0.1:27017/kaleem?replicaSet=rs0}"
MONGO_ENABLED="${MONGO_ENABLED:-1}"

# Qdrant
QDRANT_ENABLED="${QDRANT_ENABLED:-1}"
QDRANT_HTTP="${QDRANT_HTTP:-http://127.0.0.1:6333}"          # Qdrant HTTP endpoint
QDRANT_CONTAINER="${QDRANT_CONTAINER:-kaleem-qdrant}"        # Docker container name to copy snapshots from

# RabbitMQ
RABBIT_ENABLED="${RABBIT_ENABLED:-1}"
RABBIT_HTTP="${RABBIT_HTTP:-http://127.0.0.1:15672}"         # Rabbit management endpoint
RABBIT_USER="${RABBIT_USER:-kaleem}"
RABBIT_PASS="${RABBIT_PASS:-supersecret}"

# n8n (SQLite volume path)
N8N_ENABLED="${N8N_ENABLED:-1}"
N8N_VOLUME_PATH="${N8N_VOLUME_PATH:-/var/lib/docker/volumes/kaleem-n8n_n8n_data/_data}"

# Configs (paths to tar.gz)
CONFIGS_ENABLED="${CONFIGS_ENABLED:-1}"
CONFIG_PATHS="${CONFIG_PATHS:-/etc/nginx /etc/cron.d /opt/kaleem}"  # space-separated list

# MinIO mirror (optional local copy of documents)
MINIO_ENABLED="${MINIO_ENABLED:-0}"
MINIO_ALIAS="${MINIO_ALIAS:-local}"
MINIO_ENDPOINT="${MINIO_ENDPOINT:-http://127.0.0.1:9000}"
MINIO_ACCESS_KEY="${MINIO_ACCESS_KEY:-minioadmin}"
MINIO_SECRET_KEY="${MINIO_SECRET_KEY:-minioadmin}"
MINIO_SOURCE="${MINIO_SOURCE:-local/documents}"   # mc path (e.g., local/documents or alias/bucket)
...
if [[ "${MONGO_ENABLED}" == "1" ]]; then ensure_cmd mongodump; fi
