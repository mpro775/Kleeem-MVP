...
import type { MissingResponse } from "@/features/merchant/missing-responses/type";
import {
  getMissingResponses,
  resolveMissingResponse,
  bulkResolve,
  addMissingToKnowledge,
} from "@/features/merchant/missing-responses/api";
import AddToKnowledgeDialog from "@/features/merchant/missing-responses/ui/AddToKnowledgeDialog";
...
export default function MissingResponsesPage() {
  ...
  const [channel, setChannel] = useState<"all" | "telegram" | "whatsapp" | "webchat">("all");
  const [type, setType] = useState<"all" | "missing_response" | "unavailable_product">("all");
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      enqueueSnackbar((error as Error).message || 'خطأ في جلب البيانات', { variant: 'error' });
      setRows([]);
      setTotal(0);
    } finally {
      setLoading(false);
  ...
  const handleResolveOne = async (id: string) => {
    try {
      ...
      fetchData();
      enqueueSnackbar('تم التحديد كمُعالج', { variant: 'success' });
    } catch (error) {
      enqueueSnackbar((error as Error).message || 'فشل العملية', { variant: 'error' });
    }
  };
  ...
  const handleSubmitKnowledge = async (payload: {
    ...
    try {
      ...
      setCurrentRow(null);
      await fetchData();
      enqueueSnackbar('تم الإضافة للمعرفة', { variant: 'success' });
    } catch (error) {
      enqueueSnackbar((error as Error).message || 'فشل الإضافة', { variant: 'error' });
    }
  };
  ...
  const handleResolveBulk = async () => {
    ...
    try {
      ...
      setSelected(new Set());
      fetchData();
      enqueueSnackbar('تم تحديد المحددة كمُعالجة', { variant: 'success' });
    } catch (error) {
      enqueueSnackbar((error as Error).message || 'فشل العملية', { variant: 'error' });
    }
  };
  ...
  return (
    <Box sx={{ p: { xs: 1.5, md: 3 }, bgcolor: "#f9fafb", minHeight: "100dvh" }}>
      <Stack
        direction="row"
