...
export class CacheService {
  ...
  constructor(
    ...
    // الحصول على Redis instance من cache manager
    this.redis = (this.cacheManager.store as any).getClient();
  ...
  async getOrSet<T>(
    ...
    try {
      ...
      this.log.error(`getOrSet error for key ${key}:`, error);
      throw error;
    }
  }
  ...
  async invalidate(pattern: string): Promise<void> {
    try {
      ...
      this.log.error(`Cache invalidation error for pattern ${pattern}:`, error);
      throw error;
    }
  }
