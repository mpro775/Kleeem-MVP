...
export class UsageAdminController {
  ...
  @Get()
  @ApiOperation({ summary: 'قائمة استهلاك الرسائل لجميع التجار (مع الحد ونسبة الاستهلاك)' })
  @ApiResponse({ status: 200, description: 'قائمة الاستهلاك مع الحد ونسبة الاستهلاك' })
  list(
    @Query() q: QueryAdminUsageDto,
  ): Promise<{
    items: Array<{
    ...
    const params = {
      ...
      limit: q.limit ?? 30,
      page: q.page ?? 1,
      sortBy: (q.sortBy ?? 'messagesUsed') as 'messagesUsed' | 'merchantId' | 'usagePercent',
  ...
  @ApiResponse({ status: 200, description: 'قائمة التجار المتجاوزين أو القاربيين للحد' })
  getAlerts(
  ...
  @ApiResponse({ status: 200, description: 'إجمالي الرسائل وعدد التجار المستهلكين' })
  getStats(
    @Query('monthKey') monthKey?: string,
  ): Promise<{
    monthKey: string;
  ...
  async resetUsage(
    ...
    const key =
      monthKey?.match(/^\d{4}-(0[1-9]|1[0-2])$/) ? monthKey : this.usageService.monthKeyFrom();
