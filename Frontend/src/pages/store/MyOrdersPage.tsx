  ...
  useEffect(() => {
    ...
    axiosInstance
      ...
      .then(async (res) => {
        ...
        // جلب معلومات التاجر
        try {
          const merchantRes = await axiosInstance.get(`/merchants/${mid}`);
          setMerchant(merchantRes.data);
        } catch (err) {
          console.warn("Failed to fetch merchant info:", err);
          // إنشاء merchant object بسيط من البيانات المتاحة
          setMerchant({
            _id: mid,
            name: res.data.storefront?.name || "متجر",
            logoUrl: res.data.storefront?.logoUrl || "",
            phone: "",
            publicSlug: slug || "",
        ...
        // جلب معلومات المتجر وتطبيق الثيم
        try {
