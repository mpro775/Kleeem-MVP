...
export default function MissingResponsesPage() {
  ...
  const params = useMemo(
    ...
    [page, limit, resolved, channel, type, search, merchantId]
  );

  // جلب البيانات عند تغيير params فقط
  useEffect(() => {
    let cancelled = false;
  ...
  const handleResolveOne = async (id: string) => {
    try {
      ...
      // تحديث الحالة محلياً بدلاً من إعادة جلب كل البيانات
      setRows((prevRows) =>
        prevRows.map((row) =>
          row._id === id
