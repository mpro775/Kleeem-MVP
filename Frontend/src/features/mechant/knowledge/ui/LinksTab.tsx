...
export default function LinksTab({ merchantId }: { merchantId: string }) {
  ...
  const handleAddSingle = async () => {
    ...
    console.log("[LinksTab] handleAddSingle called:", value);
    if (!value) {
      console.log("[LinksTab] Empty value, returning");
      return;
    }
    if (!isValidUrl(value)) {
      console.log("[LinksTab] Invalid URL:", value);
      enqueueSnackbar("الرجاء إدخال رابط صحيح", { variant: "warning" });
      return;
    }
    setSubmitting(true);
    console.log("[LinksTab] Calling add function");
    try {
      await add(value);
      console.log("[LinksTab] Add successful");
      setSingleUrl("");
      enqueueSnackbar("تمت إضافة الرابط وقيد المعالجة", { variant: "success" });
      await Promise.all([fetchStatus(), refresh?.()]);
    } catch (e: any) {
      console.error("[LinksTab] Add error:", e);
  ...
  const statusChip = (s?: LinkItem["status"], error?: string) => {
    // "processed" و "completed" كلاهما يعني أن الرابط مكتمل
    if (s === "completed" || s === "processed") return <Chip size="small" label="مكتمل" color="success" />;
    if (s === "failed")
      return (
    ...
    // "pending" و "processing" و أي حالة أخرى تعني قيد التدريب
    return (
      <Chip
        size="small"
        label={
              ...
              <Box key={l._id}>
                ...
                >
                  <Box flex={1} minWidth={0}>
                    ...
                    {l.status !== "completed" && l.status !== "processed" && l.status !== "failed" && (
                      <LinearProgress
